...
namespace ImGuiDemoWidget
{
	...
	static void Tick(FImGuiTickContext* Context)
	{
		...
		// NOTE: The widget params uses `bSkipWindowCreation=true` which alows the widget to host multiple ImGui windows
		// This also means we have to manually sync the active state here, the menu bar item cannot check if the window is active
		if (ImGui::Begin("Unreal Demo", ImGuiSubsystem->GetMainMenuWidgetActiveState(/*World=*/nullptr, "ImGui.Demo")))
		{
			...
			{
				ImGui::Checkbox("Show ImGui Demo", &bShowImGuiDemo);
				ImGui::Checkbox("Show ImPlot Demo", &bShowImPlotDemo);				
			}
		}
		ImGui::End();
